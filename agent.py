...
class Qnetwork():
    def __init__(self, state_size, h_size, rnn_cell, myScope):
        ...
        self.inputFlat = tf.reshape(slim.flatten(self.scalarInput), [self.batch_size, self.trainLength, h_size])
        self.state_in = rnn_cell.zero_state(self.batch_size, tf.float32)
        self.rnn, self.rnn_state = tf.nn.dynamic_rnn( \
            inputs=self.inputFlat, cell=rnn_cell, dtype=tf.float32, initial_state=self.state_in, scope=myScope + '_rnn')
...
h_size = env.action  # The size of the final convolutional layer before splitting it into Advantage and Value streams.
